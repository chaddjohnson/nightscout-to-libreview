...
const getNightscoutFoodEntries = async function (
  baseUrl,
  token,
  fromDate,
  toDate
...
const getNightscoutGlucoseEntries = async function (
  baseUrl,
  token,
  fromDate,
  toDate
...
const getNightscoutInsulinEntries = async function (
  baseUrl,
  token,
  fromDate,
  toDate
  ...
  const url2 = `${baseUrl}/api/v1/treatments.json?find[created_at][$gte]=${fromDate}&find[created_at][$lte]=${toDate}&find[eventType]=Bolus&count=131072${getNightscoutToken(
    token
  )}`;
